...
layout (location = 3) uniform int group;
...
float poly6KernelConst = 315.0 / (64.0 * 3.14159265 * pow(smoothing_length, 9.0f));
float smoothing_length_second_power = pow(smoothing_length, 2.0f);

vec3 own_position = vec3(imageLoad(position_read_buffer, GIIDX));
float density = 0.0f;
...
void calcContribution(int offset)
{
	...
	if (dist == 0)
	{
		density += particle_mass * poly6KernelConst * pow(smoothing_length_second_power,3.0f);
	} else if(dist < smoothing_length)
	{
		density += particle_mass * poly6KernelConst * pow(smoothing_length_second_power - pow(dist,2.0f),3.0f);
	}
}
...
void main(void)
{
	...
	if(ID > -.9f)
	{
		...
		for(int x = -1; x < 2; x++)
		{
			for(int y = -1; y < 2; y++)
			{
				for(int z = -1; z < 2; z++)
				{
					...
					int yj = yi + y;
					int zj = zi + z;
					if(0 <= xj && xj < grid_cell_count && 0 <= yj && yj < grid_cell_count && 0 <= zj && zj < grid_cell_count) // true if valid cell
					{
						...
						if(startOffset != -1) // cell not empty
						{
							...
							imageStore(offset_texture, GIIDX * 27 + counter, ivec4(startOffset, endOffset, 0, 0));
						...
						{
							imageStore(offset_texture, GIIDX * 27 + counter, ivec4(-1));
						}
					}
					else
					{
		...
		imageStore(position_read_buffer, GIIDX, vec4(own_position,density)); // set density
	}
}